...
namespace Grimoire
{
    ...
    internal sealed partial class FA : ICloneable
    {
        /// <summary>
        ...
        public object AcceptingSymbol { get; set; }
        ...
        public static FA Clone(IEnumerable<FA> closure)
        {
            ...
            foreach (var fa in lclosure)
            {
                foreach (var trns in (IDictionary<FA, ICollection<char>>)fa.Transitions)
                    ((IDictionary<FA, ICollection<char>>)newClosure[i].Transitions).Add(
                        newClosure[lclosure.IndexOf(trns.Key)], new HashSet<char>(trns.Value));
        ...
        private string _RegexEscape(char ch)
        {
            switch (ch)
            {
                ...
                case ')':
                    return @"\)";
                default:
                    if (!char.IsLetterOrDigit(ch) && !char.IsSeparator(ch) && !char.IsPunctuation(ch) &&
                        !char.IsSymbol(ch))
                        return string.Concat("\\u", ((ushort)ch).ToString("x4"));
                    return ch.ToString();
            }
        }

        private void _ToString(StringBuilder sb, IList<FA> visited, IList<KeyValuePair<FA, FA>> loopMarkers)
        {
            if (null == visited)
                visited = new List<FA>();
            ...
            foreach (var lp in loopMarkers)
                if (FillEpsilonClosure().Contains(lp.Key))
                    sb.Append("(");
            var states = new List<FA>();
            FillEpsilonClosure(states);
            var d = new Dictionary<FA, IList<KeyValuePair<char, char>>>();
            foreach (var fa in states) fa.FillInputTransitionRangesGroupedByState(d);
            var delim = "";
            if (1 < d.Count)
                sb.Append('(');
        ...
        /// <remarks>
        ///     DOES NOT WORK YET. Mostly for debugging. This algorithm is naive and only supports |, *, (), and [] which can
        ///     make expressions very large, and unreasonable for more complicated expressions with hundreds or sometimes even
        ///     dozens of states.
        /// </remarks>
        ...
        private IEnumerable<FA> _EnumClosure(ICollection<FA> visited)
        {
            if (0 < visited.Count)
            {
        ...
        private IEnumerable<FA> _EnumClosure2(ICollection<FA> visited)
        {
        ...
        /// </summary>
        /// <remarks>
        ///     Usually, <see cref="FA.FillClosure(IList{FA})" /> will be faster. The exception is when a large closure only
        ///     needs to be partially enumerated. This is wrapped with a <see cref="System.Collections.Generic.IList{FA}" />for
        ///     convenience but the list doesn't have the performance properties of a standard list.
        /// </remarks>
        public IList<FA> Closure => _EnumClosure(new List<FA>()).AsList();
        ...
        private IEnumerable<FA> _EnumEpsilonClosure(IList<FA> visited)
        {
            if (visited.Contains(this))
                yield break;
            visited.Add(this);
        ...
        /// </summary>
        public IList<FA> EpsilonClosure => _EnumEpsilonClosure(new List<FA>()).AsList();
        ...
        /// </summary>
        public bool IsNeutral => null == AcceptingSymbol && 0 == Transitions.Count && 1 == EpsilonTransitions.Count;
        ...
        private static IEnumerable<FA> _EnumNeutrals(IEnumerable<FA> closure)
        {
            foreach (var fa in closure)
        ...
        /// </summary>
        public IList<FA> Neutrals => _EnumNeutrals(FillClosure()).AsList();

        /// <summary>
        ///     Returns true if the state has no transitions.
        /// </summary>
        public bool IsFinal => 0 == Transitions.Count && 0 == EpsilonTransitions.Count;
        ...
        private static IEnumerable<FA> _EnumFinals(IEnumerable<FA> closure)
        {
            foreach (var fa in closure)
                if (null != fa.AcceptingSymbol)
                    yield return fa;
        ...
        /// </summary>
        public IList<FA> Finals => _EnumFinals(FillClosure()).AsList();

        /// <summary>
        ///     Returns true if the state is an accepting state
        /// </summary>
        public bool IsAccepting => null != AcceptingSymbol;
        ...
        private static IEnumerable<FA> _EnumAccepting(IEnumerable<FA> closure)
        {
            foreach (var fa in closure)
                if (null != fa.AcceptingSymbol)
                    yield return fa;
        ...
        /// </summary>
        public IList<FA> Accepting => _EnumAccepting(FillClosure()).AsList();
        ...
        /// </summary>
        public IList<object> AcceptingSymbols => _EnumAcceptingSymbols(FillClosure()).AsList();

        /// <summary>
        ///     Fills a list with all accepting symbols reachable from the specified closure.
        /// </summary>
        /// <param name="closure">
        ///     The closure of all states, or alternatively, a collection of states from which to retrieve
        ///     accepting symbols.
        /// </param>
        /// <param name="result">The list to fill</param>
        ...
        /// </summary>
        public bool IsDfa => _IsDfa(FillClosure());
        ...
        private static bool _IsDfa(IEnumerable<FA> closure)
        {
        ...
        /// <remarks>
        ///     The capture buffer will contain all characters consumed. After reading, the current character will either be
        ///     the character immediately following the match, or the error character.
        /// </remarks>
        public bool TryRead(ParseContext pc)
        {
            pc.EnsureStarted();
            var states = FillEpsilonClosure();
            var isAccepting = IsAnyAccepting(states);
            if (-1 == pc.Current)
        ...
        private static object _GetAcceptingSymbol(IEnumerable<FA> states, bool firstOnly = false)
        {
        ...
        private static void _ExpectingDfat((KeyValuePair<char, char>[] Ranges, int Destination)[] trns, ParseContext pc)
        {
            var ranges = new List<KeyValuePair<char, char>>();
            for (var i = 0; i < trns.Length; ++i)
            {
                var trn = trns[i];
                for (var j = 0; j < trn.Item1.Length; ++j)
        ...
        private static int _GetDfatTransition((KeyValuePair<char, char>[] Ranges, int Destination)[] trns, char ch)
        {
            for (var i = 0; i < trns.Length; ++i)
            {
                var trn = trns[i];
        ...
        /// <param name="sb">
        ///     The <see cref="System.Text.StringBuilder" /> that holds the result. If null, a new
        ///     <see cref="System.Text.StringBuilder" /> will be created. Otherwise the passed in
        ///     <see cref="System.Text.StringBuilder" /> will be cleared.
        /// </param>
        /// <returns>
        ///     A <see cref="System.Collections.Generic.KeyValuePair" /> that contains the accepting symbol(s) and the
        ///     captured string.
        /// </returns>
        /// <remarks>The capture buffer is not affected.</remarks>
        public static KeyValuePair<object, string> Lex(
            (object Accept, (KeyValuePair<char, char>[] Ranges, int Destination)[])[] dfaTable, ParseContext pc,
            StringBuilder sb = null)
        {
            return Lex<object>(dfaTable, pc, sb);
        }
        ...
        /// </returns>
        /// <remarks>The capture buffer is not affected.</remarks>
        public static KeyValuePair<TAccept, string> Lex<TAccept>(
            (TAccept Accept, (KeyValuePair<char, char>[] Ranges, int Destination)[])[] dfaTable, ParseContext pc,
            StringBuilder sb = null)
        {
            if (null == sb)
        ...
        public KeyValuePair<object, string> Lex(ParseContext pc, StringBuilder sb = null)
        {
            ...
            if (-1 == pc.Current)
            {
                if (!isAccepting)
                {
                    var ex = new List<int>();
                    foreach (var s in states)
                    foreach (var ch in s.Transitions.Keys)
                        ex.Add(ch);
                    pc.Expecting(ex.ToArray());
                }

                return new KeyValuePair<object, string>(_GetAcceptingSymbol(states, true),
                    sb.ToString()); // accept an empty string?
            }
            ...
            while (0 < states.Count)
            {
                ...
                if (-1 == pc.Advance())
                {
                    if (!isAccepting)
                    {
                        var ex = new List<int>();
                        foreach (var s in states)
                        foreach (var ch2 in s.Transitions.Keys)
                            ex.Add(ch2);
                        pc.Expecting(ex.ToArray());
                    }
        ...
        public bool TryLex(ParseContext pc, out KeyValuePair<object, string> result)
        {
            ...
            if (-1 == pc.Current)
            {
                ...
                result = new KeyValuePair<object, string>(_GetAcceptingSymbol(states, true),
                    pc.GetCapture(l)); // accept an empty string?
                return true;
            }
            ...
            result = new KeyValuePair<object, string>(_GetAcceptingSymbol(states, true),
                pc.GetCapture(l)); // && 0==states.Count
            return true;
        }
        ...
        /// <param name="name">The name of FSM. This name will be appended to the method name.</param>
        /// <param name="access">The access level of the generated function. Can be "public", "private", "internal" or "" or null</param>
        public static void WriteCSharpLexMethodTo(IEnumerable<FA> closure, TextWriter writer, string name,
            string access = null)
        {
        ...
        /// <param name="name">The name of FSM. This name will be appended to the method name.</param>
        /// <param name="access">The access level of the generated function. Can be "public", "private", "internal" or "" or null</param>
        public static void WriteTryLexMethodTo(IEnumerable<FA> closure, TextWriter writer, string name,
            string access = null)
        {
            _WriteLexMethodTo(closure, writer, name, access, true);
        }

        public static void WriteCSharpDfaTableCreationExpressionTo(TextWriter writer,
            (object Accept, (KeyValuePair<char, char>[] Ranges, int Destination)[])[] dfaTable)
        {
            WriteCSharpDfaTableCreationExpressionTo<object>(writer, dfaTable);
        }

        public static void WriteCSharpDfaTableCreationExpressionTo<TAccept>(TextWriter writer,
            (TAccept Accept, (KeyValuePair<char, char>[] Ranges, int Destination)[])[] dfaTable)
        {
            writer.WriteLine(string.Concat("new (", typeof(TAccept).FullName,
                " Accept, (System.Collections.Generic.KeyValuePair<char, char>[] Ranges, int Destination)[])[] {"));
            var delim = "";
            for (var i = 0; i < dfaTable.Length; ++i)
            {
                ...
                writer.WriteLine(",");
                writer.WriteLine(
                    "\t\tnew (System.Collections.Generic.KeyValuePair<char,char>[] Ranges,int Destination)[] {");
                var delim2 = "";
                for (var j = 0; j < dfaEntry.Item2.Length; ++j)
                {
                    writer.Write(delim2);
                    ...
                    var delim3 = "";
                    for (var k = 0; k < trns.Item1.Length; ++k)
                    {
                        writer.Write(delim3);
                        var rg = trns.Item1[k];
		...
		static CodeMemberMethod _GenerateLexMethod(IEnumerable<FA> closure, string name, MemberAttributes attributes, bool tryLex)
		{
			...
			{
				ld.InitExpression =
 new CodePropertyReferenceExpression(new CodePropertyReferenceExpression(pcr, "Capture"), "Length");
				stmts.Add(ld);
			}
			int i = 0;
			foreach (var ffa in closure)
			{
				...
				{
					var pccr =
 new CodePropertyReferenceExpression(new CodeArgumentReferenceExpression(result.Parameters[0].Name), "Current");
				...
				if (ffa.IsAccepting)
				{
					...
					{
						capt =
 new CodeMethodInvokeExpression(new CodeArgumentReferenceExpression(result.Parameters[1].Name), "ToString");
					}
					else
					{
...
#endif
        private static void _WriteLexMethodTo(IEnumerable<FA> closure, TextWriter w, string name, string access,
            bool tryLex)
        {
            FA fa;
            fa = closure.First();
            ...
            var al = FillAcceptingSymbols(closure);
            for (var j = 0; j < al.Count; ++j)
            {
                var e = al[j] as IEnumerable;
                if (al[j] is string) e = null;
                if (null != e) al[j] = e.First();
            }

            var t = al.InferElementType();
            ...
            if (!tryLex)
                w.Write(
                    "System.Collections.Generic.KeyValuePair<{0},string> Lex{1}(Grimoire.ParseContext pc, System.Text.StringBuilder sb=null) ",
                    t.FullName, name);
            else
                w.Write(
                    "bool TryLex{1}(Grimoire.ParseContext pc, out System.Collections.Generic.KeyValuePair<{0},string> result) ",
                    t.FullName, name);
            w.WriteLine("{");
            ...
            foreach (var ffa in closure)
            {
                if (0 != i || ffa.IsLoop)
                    w.WriteLine("q{0}:", i);
                var itr = ffa.FillInputTransitionRangesGroupedByState();
                var ranges = new List<KeyValuePair<char, char>>();
                foreach (var kvp in itr)
                {
                    w.Write("\tif(");
                ...
                else
                {
                    ...
                    {
                        w.WriteLine("\tresult=default(System.Collections.Generic.KeyValuePair<{0},string>);",
                            t.FullName);
                        w.WriteLine("\treturn false;");
                    }
                }
        ...
        /// <summary>
        ///     Returns a <see cref="IDictionary{FA,IList{KeyValuePair{Char,Char}}}" />, keyed by state, that contains all of the
        ///     outgoing local input transitions, expressed as a series of ranges
        /// </summary>
        ...
        public IDictionary<FA, IList<KeyValuePair<char, char>>> FillInputTransitionRangesGroupedByState(
            IDictionary<FA, IList<KeyValuePair<char, char>>> result = null)
        {
            if (null == result)
        ...
        /// <summary>
        ///     Fills a list with the references to <paramref name="target" /> from the state machine indicated by
        ...
        /// <param name="result">
        ///     A list to fill with the states that refer to <paramref name="target" />, or null for a new list to
        ///     be created
        /// </param>
        ...
        public bool IsDuplicate(FA rhs)
        {
            return null != rhs &&
                   ...
                   _SetComparer.Default.Equals((IDictionary<FA, ICollection<char>>)Transitions,
                       (IDictionary<FA, ICollection<char>>)rhs.Transitions);
        }

        /// <summary>
        ///     Fills a <see cref="IDictionary{FA, ICollection{FA}}" /> with all duplicates in this machine, grouped by each
        ///     duplicate state
        /// </summary>
        ...
        /// <summary>
        ///     Fills a <see cref="IDictionary{FA, ICollection{FA}}" /> with all duplicates in the closure, grouped by each
        ///     duplicate state
        /// </summary>
        /// <param name="closure">The closure of all states</param>
        /// <param name="result">The <see cref="IDictionary{FA, ICollection{FA}}" /> to fill, or null to create a new one</param>
        /// <returns>The <see cref="IDictionary{FA, ICollection{FA}}" /> containing the duplicates</returns>
        public static IDictionary<FA, ICollection<FA>> FillDuplicatesGroupedByState(IEnumerable<FA> closure,
            IDictionary<FA, ICollection<FA>> result)
        {
            if (null == closure)
                throw new ArgumentNullException(nameof(closure));
            if (null == result)
                result = new Dictionary<FA, ICollection<FA>>();
            var cl = closure as IList<FA> ?? new List<FA>(closure);
            var c = cl.Count;
            for (var i = 0; i < c; i++)
            {
                var s = cl[i];
        ...
        /// <returns>
        ///     The list of <see cref="KeyValuePair{FA,FA}" /> entries where Key is the start of the loop, and Value is the
        ///     end of the loop. Keys may be duplicated.
        /// </returns>
        public static IList<KeyValuePair<FA, FA>> FillLoops(IEnumerable<FA> closure,
            IList<KeyValuePair<FA, FA>> result = null)
        {
            if (null == result)
            ...
            {
                foreach (var rfa in FillReferences(cl, ffa))
                {
                ...
                if (0 < dups.Count)
                {
                    foreach (var de in dups)
                    {
                        ...
                        for (var i = 0; i < c; ++i)
                        {
                            ...
                            var lc = s.EpsilonTransitions.Count;
                            for (var j = 0; j < lc; ++j)
        ...
        private static IEnumerable<char> _ExpandRange(KeyValuePair<char, char> range)
        {
        ...
        private static IEnumerable<char> _ExpandRanges(IEnumerable<KeyValuePair<char, char>> ranges)
        {
            foreach (var range in ranges)
            foreach (var ch in _ExpandRange(range))
                yield return ch;
        }

        private static IEnumerable<KeyValuePair<char, char>> _GetRanges(IEnumerable<char> sortedString)
        {
            var first = '\0';
            var last = '\0';
            using (var e = sortedString.GetEnumerator())
            {
                var moved = e.MoveNext();
                while (moved)
                {
                    first = last = e.Current;
                    while ((moved = e.MoveNext()) && (e.Current == last || e.Current == last + 1)) last = e.Current;
                    yield return new KeyValuePair<char, char>(first, last);
                }
            }
        }

        private static IEnumerable<KeyValuePair<char, char>> _NotRanges(IEnumerable<KeyValuePair<char, char>> ranges)
        {
            ...
            using (var e = ranges.GetEnumerator())
            {
                ...
                while (e.MoveNext())
                {
                    ...
                    if (unchecked((char)(last + 1)) < e.Current.Key)
                        yield return new KeyValuePair<char, char>(unchecked((char)(last + 1)),
                            unchecked((char)(e.Current.Key - 1)));
                    last = e.Current.Value;
                }
        ...
        private static void _NormalizeRanges(List<KeyValuePair<char, char>> ranges)
        {
        ...
        private static char _ReadRangeChar(IEnumerator<char> e)
        {
            char ch;
            if ('\\' != e.Current || !e.MoveNext()) return e.Current;
            ch = e.Current;
            switch (ch)
            {
                case 't':
        ...
        private static IEnumerable<KeyValuePair<char, char>> _ParseRanges(IEnumerable<char> charRanges, bool normalize)
        {
            if (!normalize) return _ParseRanges(charRanges);
        ...
        private static IEnumerable<KeyValuePair<char, char>> _ParseRanges(IEnumerable<char> charRanges)
        {
        ...
        private static void _Literal(FA first, FA final, IEnumerable<char> @string)
        {
            var current = first;
        ...
        public static FA Literal(object accepting, params char[] @string)
        {
            return Literal(@string, accepting);
        }
        ...
        private static void _Set(FA first, FA final, IEnumerable<char> inputs)
        {
        ...
        {
            return Set(_ExpandRanges(charRanges), accepting);
        }

        public static FA Set(object accepting, params char[] inputs)
        {
            return Set(inputs, accepting);
        }
        ...
        public static FA Or(object accepting, params FA[] exprs)
        {
            return Or(exprs, accepting);
        }
        ...
        /// <param name="exprs">The expressions to match</param>
        /// <returns>A new machine that matches the <paramref name="exprs" /></returns>
        public static FA Concat(object accepting, params FA[] exprs)
        {
            return Concat(exprs, accepting);
        }
        ...
        public static FA Concat(IEnumerable<FA> exprs, object accepting = null)
        {
            ...
            foreach (var afa in left.Accepting) afa.AcceptingSymbol = accepting;
            return result;
        }
                ...
                case 0:
                    switch (max)
                    {
                        ...
                        default:
                            ...
                            fa = Optional(fa);
                            l.Add(fa);
                            for (var i = 1; i < max; ++i) l.Add(fa.Clone());
                ...
                default:
                    switch (max)
                    {
                        ...
                        default:
                            ...
                            return Concat(accepting, Repeat(fa.Clone(), min, min, accepting),
                                Repeat(Optional(fa.Clone()), max - min, max - min, accepting));
                    }
            }
        ...
        /// <param name="createDefaultAcceptConstants">
        ///     True if the default FA instances that are used for accepting states (when
        ///     unspecified) should be replaced instead with integer constants. States with explicitly set accepting symbols
        ///     shouldn't be affected by this unless they use FA states as accepting symbols.
        /// </param>
        ...
        /// <param name="exprs">The expressions to compose the lexer of. Each expression should produce a token</param>
        /// <returns>An FA that will lex the specified expressions</returns>
        public static FA Lexer(params FA[] exprs)
        {
            return Lexer(exprs, true);
        }
        ...
        private static void _AppendRangesTo(StringBuilder builder, IEnumerable<KeyValuePair<char, char>> ranges)
        {
            foreach (var range in ranges)
                _AppendRangeTo(builder, range);
        }

        private static void _AppendRangeTo(StringBuilder builder, KeyValuePair<char, char> range)
        {
        ...
        private static void _AppendRangeCharTo(StringBuilder builder, char rangeChar)
        {
            switch (rangeChar)
            {
                case '-':
                ...
                case '\b':
                    ...
                    return;
                default:
                    if (!char.IsLetterOrDigit(rangeChar) && !char.IsSeparator(rangeChar) &&
                        !char.IsPunctuation(rangeChar) && !char.IsSymbol(rangeChar))
                    {
                        builder.Append("\\u");
                        builder.Append(((ushort)rangeChar).ToString("x4"));
                    }
                    else
                    {
                        builder.Append(rangeChar);
        ...
        private static bool _TryForwardNeutral(FA fa, out FA result)
        {
        ...
        private static FA _ForwardNeutrals(FA fa)
        {
            var result = fa;
        ...
        public static FA ToDfa(IEnumerable<FA> closure)
        {
            ...
            var result = dfa; // store the initial state for later, so we can return it.
        ...
        {
            return ToDfaTable<TAccept>(FillClosure());
        }
        ...
        /// <param name="closure">The set of all states</param>
        /// <returns>A complex tuple representing the DFA table</returns>
        public static (object Accept, (KeyValuePair<char, char>[] Ranges, int Destination)[])[] ToDfaTable(
            IEnumerable<FA> closure)
        {
            return ToDfaTable<object>(closure);
        }
        ...
        /// <param name="closure">The set of all states</param>
        /// <returns>A complex tuple representing the DFA table</returns>
        public static (TAccept Accept, (KeyValuePair<char, char>[] Ranges, int Destination)[])[] ToDfaTable<TAccept>(
            IEnumerable<FA> closure)
        {
            if (!_IsDfa(closure))
            {
            ...
            foreach (var ffa in cl)
            {
                ...
                var asym = _GetAcceptingSymbol(new[] { ffa }, true);
                if (null == asym)
                {
        ...
        public static void WriteDotTo(IEnumerable<FA> closure, TextWriter writer, DotGraphOptions options = null)
        {
            ...
            foreach (var ffa in closure)
            {
                ...
                if (null != options.DebugString)
                {
                    if (null != toStates && toStates.Contains(ffa)) writer.Write("color=green,");
        ...
        private static string _EscapeLabel(string label)
        {
        ...
        /// </summary>
        /// <param name="format">
        ///     The output format. The format to render can be any supported dot output format. See dot command
        ///     line documation for details.
        /// </param>
        /// <param name="options">A <see cref="DotGraphOptions" /> instance with any options, or null to use the defaults</param>
        ...
        public Stream RenderToStream(string format, bool copy = false, DotGraphOptions options = null)
        {
            ...
            using (var proc = Process.Start(psi))
            {
                WriteDotTo(proc.StandardInput, options);
                proc.StandardInput.Close();
                if (!copy) return proc.StandardOutput.BaseStream;
        ...
        public static int _ParseEscape(ParseContext pc)
        {
            ...
            switch (pc.Current)
            {
                ...
                case 'x':
                    if (-1 == pc.Advance())
                        return 'x';
                    var b = _FromHexChar((char)pc.Current);
                    b <<= 4;
                    if (-1 == pc.Advance())
                        return (char)b;
        ...
        public static FA Parse(IEnumerable<char> @string, object accepting = null)
        {
            return Parse(ParseContext.Create(@string), accepting);
        }
        ...
        /// <param name="accepting">The symbol reported when accepting the specified expression</param>
        /// <returns>A new machine that matches the regular expression</returns>
        public static FA Parse(TextReader reader, object accepting = null)
        {
            return Parse(ParseContext.Create(reader), accepting);
        }
            ...
            while (true)
                switch (pc.Current)
                {
                    ...
                    case '.':
                        ...
                        current = Set(new[] { new KeyValuePair<char, char>(char.MinValue, char.MaxValue) }, accepting);
                        switch (pc.Current)
                        {
                            case '*':
                                current = Kleene(current, accepting);
                                pc.Advance();
                                break;
                            case '+':
                                current = Repeat(current, accepting);
                                pc.Advance();
                                break;
                            case '?':
                                current = Optional(current, accepting);
                                pc.Advance();
                                break;
                        }
                        ...
                        if (-1 != (ch = _ParseEscape(pc)))
                        {
                            ...
                            switch (pc.Current)
                            {
                                case '*':
                                    ...
                                    next = Kleene(next, accepting);
                                    pc.Advance();
                                    break;
                                case '+':
                                    next = new FA();
                                    next.Transitions.Add((char)ch, new FA(accepting));
                                    next = Repeat(next, accepting);
                                    pc.Advance();
                                    break;
                                case '?':
                                    next = new FA();
                                    next.Transitions.Add((char)ch, new FA(accepting));
                                    next = Optional(next, accepting);
                                    pc.Advance();
                                    break;
                                default:
                    ...
                    case '(':
                        ...
                        f = Concat(accepting, f, current);
                        break;
                    case '|':
                        if (-1 != pc.Advance())
                        {
                            current = Parse(pc, accepting);
                            result = Or(accepting, result, current);
                        }
                        else
                        {
                            current = current.Finals.First();
                            result = Optional(result, accepting);
                        }
                    ...
                    case '[':
                        ...
                        var r = !not && "." == pc.Capture
                            ? new[] { new KeyValuePair<char, char>(char.MinValue, char.MaxValue) }
                            : _ParseRanges(pc.Capture, true);
                        if (not)
                            r = _NotRanges(r);
                        f = current.Accepting.First();
                        current = Set(r, accepting);
                        switch (pc.Current)
                        {
                            case '*':
                                current = Kleene(current, accepting);
        ...
        private class _TrnsDic : IDictionary<char, FA>, IDictionary<FA, ICollection<char>>
        {
            private readonly IDictionary<FA, ICollection<char>> _inner = new Dictionary<FA, ICollection<char>>();
            ...
            public ICollection<char> Keys => _EnumKeys().AsCollection();

            public ICollection<FA> Values => _EnumValues().AsCollection();
            ...
            ICollection<FA> IDictionary<FA, ICollection<char>>.Keys => _inner.Keys;
            ICollection<ICollection<char>> IDictionary<FA, ICollection<char>>.Values => _inner.Values;
            int ICollection<KeyValuePair<FA, ICollection<char>>>.Count => _inner.Count;

            ICollection<char> IDictionary<FA, ICollection<char>>.this[FA key]
            {
                get => _inner[key];
            ...
            void ICollection<KeyValuePair<FA, ICollection<char>>>.CopyTo(KeyValuePair<FA, ICollection<char>>[] array,
                int arrayIndex)
            {
            ...
            IEnumerator<KeyValuePair<FA, ICollection<char>>> IEnumerable<KeyValuePair<FA, ICollection<char>>>.
                GetEnumerator()
            {
                return _inner.GetEnumerator();
            }

            private IEnumerable<char> _EnumKeys()
            {
                foreach (var trns in _inner)
            ...
            private IEnumerable<FA> _EnumValues()
            {
                foreach (var trns in _inner)
                foreach (var key in trns.Value)
        ...
        private sealed class _SetComparer : IEqualityComparer<IList<FA>>, IEqualityComparer<ICollection<FA>>,
            IEqualityComparer<IDictionary<char, FA>>
        {
