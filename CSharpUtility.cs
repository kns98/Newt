...
namespace Grimoire
{
    ...
    internal static class CSharpUtility
    {
        public static void WriteCSharpLiteralTo(TextWriter writer, object val)
        {
            ...
            if (val is short || val is ushort || val is int || val is uint || val is ulong || val is long ||
                val is byte || val is sbyte || val is float || val is double || val is decimal)
            {
                writer.Write(val);
                return;
            }
            ...
            if (null != conv)
            {
                if (conv.CanConvertTo(typeof(InstanceDescriptor)))
                {
                    ...
                    if (!desc.IsComplete)
                        throw new NotSupportedException(string.Format("The type \"{0}\" could not be serialized.",
                    ...
                    else
                    {
                        throw new NotSupportedException(string.Format(
                            "The instance descriptor for type \"{0}\" is not supported.", val.GetType().FullName));
                    }
                }
                else
                {
        ...
        private static void _WriteCSharpCharPartTo(TextWriter writer, char ch)
        {
            switch (ch)
            {
                case '\'':
                ...
                case '\u0085':
                    writer.Write("\\u");
                    writer.Write(((ushort)ch).ToString("x4"));
                    break;
                default:
        ...
        {
            if (IsKeyword(identifier) || _IsPrefixTwoUnderscore(identifier)) return "@" + identifier;
            return identifier;
        }

        private static bool _IsPrefixTwoUnderscore(string value)
        {
            if (value.Length < 3)
                return false;
        ...
        private static bool _FixedStringLookupContains(string[] array, string value)
        {
            ...
            while (pos < value.Length)
            {
                ...
                if (max - min <= 1)
                {
                    ...
                    if (searchChar != array[min][pos]) return false;
                    pos++;
                    continue;
                }

                // There are multiple items to search, use binary search to find one of the hits
                if (!_FindCharacter(array, searchChar, pos, ref min, ref max)) return false;
        ...
        private static bool _FindCharacter(string[] array, char value, int pos, ref int min, ref int max)
        {
            var index = min;
            while (min < max)
            {
                ...
                if (value == comp)
                {
                    ...
                    var newMin = index;
                    while (newMin > min && array[newMin - 1][pos] == value) newMin--;
                    min = newMin;
                    var newMax = index + 1;
                    while (newMax < max && array[newMax][pos] == value) newMax++;
                    max = newMax;
                    return true;
                }
        ...
        internal static bool _FixedStringLookup(string[][] lookupTable, string value)
        {
            ...
            if (length <= 0 || length - 1 >= lookupTable.Length) return false;
