...
namespace Grimoire
{
    internal class TableDrivenLL1Parser : LLParser
    {
        private int _eosId = -1;
        private int _errorId = -1;
        private readonly StringBuilder _lexerBuffer = new StringBuilder();
        private LLNodeType _nodeType = LLNodeType.Initial;
        ...
        protected int EosId
        {
            get
            {
                if (-1 == _eosId) _eosId = GetSymbolId("#EOS");
                return _eosId;
            }
        }
        ...
        protected int ErrorId
        {
            get
            {
                if (-1 == _errorId) _errorId = GetSymbolId("#ERROR");
                return _errorId;
            }
        }

        protected (int Accept, ((char First, char Last)[] Ranges, int Destination)[] Transitions, int[] PossibleAccepts)
            [] LexTable { get; }
        ...
        protected int[]
            CollapsedNonTerminals { get; } // we may end up holding other things from attributes here too eventually
        ...
        protected (int SymbolId, string Value, int Line, int Column, long Position, (int First, int Last)[]
            ExpectingRanges, int[] ExpectingSymbols) Token { get; private set; }

        protected (int SymbolId, string Value, int Line, int Column, long Position, (int First, int Last)[]
            ExpectingRanges, int[] ExpectingSymbols) ErrorToken { get; private set; }
        ...
        protected (int SymbolId, string Value, (int First, int Last)[] ExpectingRanges, int[] ExpectingSymbols) Lex(
            StringBuilder lexerBuffer = null)
        {
            return ParserUtility.Lex2(LexTable, ErrorId, ParseContext, lexerBuffer);
        }
            ...
            if (0 < Stack.Count)
            {
                ...
                if (null != d)
                {
                    ...
                    if (-1 != rule.Left)
                    {
                        ...
                        sid = ~sid;
                        Stack.Push(sid);
                        for (var j = rule.Right.Length - 1; j >= 0; --j)
                        {
                            sid = rule.Right[j];
                            Stack.Push(sid);
                        }
        ...
        protected void UpdateNodeType(LLNodeType nodeType)
        {
            _nodeType = nodeType;
        }

        protected void UpdateSymbolId(int symbolId)
        {
            _symbolId = symbolId;
        }
        ...
        protected void NextToken()
        {
            while (true)
            {
                ...
                {
                    Token = (SymbolId: EosId, Value: null, ParseContext.Line, ParseContext.Column,
                        ParseContext.Position, null, null);
                    break;
                }

                {
                    ...
                    Token = (t.SymbolId, t.Value, Line: l, Column: c, Position: pos, t.ExpectingRanges,
                        t.ExpectingSymbols);
                }
                var blockEnd = GetBlockEnd(Token.SymbolId);
                if (null != blockEnd)
                {
                    var l = ParseContext.CaptureBuffer.Length;
                    if (!ParseContext.TryReadUntil(blockEnd))
                        ParseContext.Expecting();
                    Token = (Token.SymbolId, Token.Value + ParseContext.GetCapture(l), Token.Line, Token.Column,
                        Token.Position, Token.ExpectingRanges, Token.ExpectingSymbols);
                }
        ...
        protected bool Panic()
        {
            ...
            {
                first = false;
                ErrorToken = (ErrorId, string.Concat(ErrorToken.Value, Token.Value), l, c, pos, Token.ExpectingRanges,
                    Token.ExpectingSymbols);
                if (-1 == ParseContext.Current)
                {
                    Token = (SymbolId: EosId, Value: "", ParseContext.Line, ParseContext.Column, ParseContext.Position,
                        null, null);
                }
                else
                {
