...
namespace Grimoire
{
        ...
        static class CollectionUtility
    {
        ...
        public static IList AsReadOnly(IList list)
        {
            return list.IsReadOnly ? list : new ReadOnlyListAdapter(list);
        }

        public static ICollection AsReadOnly(ICollection collection)
        {
        ...
        {
            return list.IsReadOnly ? list : new ReadOnlyListAdapter<T>(list);
        }

        public static ICollection<T> AsReadOnly<T>(ICollection<T> collection)
        {
            return collection.IsReadOnly ? collection : new ReadOnlyCollectionAdapter<T>(collection);
        }
        ...
        /// <returns>
        ///     True if the enumeration is null, or if enumerating ends before the first element. Otherwise, this method
        ///     returns true.
        /// </returns>
        ...
        public static bool StartsWith<T>(this IEnumerable<T> collection, IEnumerable<T> values,
        ...
        public static IList<T> GetLongestCommonPrefix<T>(this IEnumerable<IList<T>> ss)
        {
            ...
            foreach (var list2 in ss)
                ...
                {
                    var pfx = GetCommonPrefix(new[] { list, list2 });
        ...
        public static int CopyTo<T>(this IEnumerable<T> source, T[] destination, int destinationStartIndex)
        {
            ...
            if (destinationStartIndex < destination.GetLowerBound(0) ||
                destinationStartIndex > destination.GetUpperBound(0))
                throw new ArgumentOutOfRangeException("startIndex");
            var i = destinationStartIndex;
        ...
        {
            foreach (var o in collection) yield return (T)o;
        }

        public static IEnumerable<T> Convert<T>(this IEnumerable collection)
        {
            var t = typeof(T);
            foreach (var o in collection) yield return (T)System.Convert.ChangeType(o, t);
        }

        public static int IndexOf(this IEnumerable collection, object item)
        {
            var result = 0;
            foreach (var cmp in collection)
            {
                if (Equals(item, cmp))
                    return result;
                ++result;
        ...
        public static IEnumerable<TOutput> ForEach<TInput, TOutput>(this IEnumerable<TInput> collection,
            Func<TInput, TOutput> @do)
        {
            foreach (var item in collection)
        ...
        public static IEnumerable<TOut> ForEachTransform<TIn, TOut>(this IEnumerable<TIn> collection,
            Func<TIn, TOut> @do)
        {
            foreach (var item in collection)
                yield return @do(item);
        ...
        {
            var result = _GetCommonType(NonNulls(GetTypes(items)));
        ...
        private static Type _GetCommonType(IEnumerable<Type> types)
        {
            ...
            while (!checkPass)
            {
                ...
                for (var i = 1; i < temp.Count; i++)
                    ...
                    else
                    {
                        ...
                        if (tested.BaseType.Equals(temp[i]))
                        {
                            for (var j = 0; j <= i - 1; j++) temp[j] = temp[j].BaseType;
                            checkPass = false;
                            break;
                        }
                        ...
                        for (var j = 0; j <= i; j++) temp[j] = temp[j].BaseType;
                        checkPass = false;
                        break;
                    }
        ...
        public static bool Equals<TKey, TValue>(this ICollection<KeyValuePair<TKey, TValue>> lhs,
            ICollection<KeyValuePair<TKey, TValue>> rhs)
        {
            if (ReferenceEquals(lhs, rhs))
                return true;
            if (ReferenceEquals(null, lhs) || ReferenceEquals(null, rhs))
        ...
        public static IEnumerable<IList<T>> Split<T>(this IEnumerable<T> collection, T delim,
            IEqualityComparer<T> equalityComparer = null)
        {
            if (null == equalityComparer)
                equalityComparer = EqualityComparer<T>.Default;
        ...
        public static IEnumerable<T> Replace<T>(this IEnumerable<T> collection, T oldValue, IEnumerable<T> newValues,
            IEqualityComparer<T> equalityComparer = null)
        {
        ...
        {
            private readonly IList<KeyValuePair<TKey, TValue>> _inner = new List<KeyValuePair<TKey, TValue>>();
            ...
            public ICollection<TKey> Keys => _EnumKeys().AsCollection();

            public ICollection<TValue> Values => _EnumValues().AsCollection();
            ...
            private static void _ThrowReadOnly()
            {
        ...
        private sealed class ListAdapter<T> : IList<T>
        {
            private readonly IEnumerable<T> _inner;
        ...
        private sealed class ReadOnlyCollectionAdapter : ICollection
        {
        ...
        private sealed class ReadOnlyCollectionAdapter<T> : ICollection<T>
        {
        ...
        private sealed class ReadOnlyListAdapter : IList
        {
