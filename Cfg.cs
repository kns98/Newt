...
namespace Grimoire
{
    ...
    internal partial class Cfg : IEquatable<Cfg>, ICloneable, ISymbolResolver
    {
        /// <summary>
        ///     Indicates sets of attributes by non-terminal that may further specify or otherwise modify parsing
        /// </summary>
        public IDictionary<string, IDictionary<string, object>> AttributeSets { get; } =
        ...
        /// </summary>
        public IList<string> NonTerminals => _EnumNonTerminals().AsList();

        /// <summary>
        ///     Provides a read-only list of all terminals in the grammar.
        /// </summary>
        public IList<string> Terminals => _EnumNonTerminals().AsList();

        /// <summary>
        ///     Provides a read-only list of all symbols in the grammar.
        /// </summary>
        public IList<string> Symbols => _EnumSymbols().AsList();
        ...
        /// <returns>
        ///     An integer that can be used to refer to the symbol, or less than zero to indicate the symbol was not present
        ///     in the CFG.
        /// </returns>
        ...
        public IDictionary<string, ICollection<string>> FillFirsts(
            IDictionary<string, ICollection<string>> result = null)
        {
            if (null == result)
                result = new Dictionary<string, ICollection<string>>();
            foreach (var t in _EnumTerminals())
                if (!Equals("#ERROR", t))
                    result.Add(t, new HashSet<string>(new[] { t }));
            var ic = Rules.Count;
        ...
        public IDictionary<CfgRule, ICollection<string>> FillPredict(
            IDictionary<string, ICollection<string>> firsts = null,
            IDictionary<string, ICollection<string>> follows = null,
            IDictionary<CfgRule, ICollection<string>> result = null)
        {
            if (null == result)
        ...
        public IDictionary<CfgRule, ICollection<CfgRule>> FillFirstFirstConflicts(
            IDictionary<string, ICollection<string>> firsts = null,
            IDictionary<CfgRule, ICollection<CfgRule>> result = null)
        {
            if (null == result)
        ...
        public IDictionary<string, ICollection<CfgRule>> FillFirstFollowsConflicts(
            IDictionary<string, ICollection<string>> firsts = null,
            IDictionary<string, ICollection<string>> follows = null,
            IDictionary<string, ICollection<CfgRule>> result = null)
        {
            if (null == result)
        ...
        public IList<CfgMessage> EliminateFirstFirstConflicts()
        {
            ...
            foreach (var nt in new List<string>(_EnumNonTerminals()))
            {
                ...
                while (true)
                {
                    ...
                    foreach (var rule in rules)
                        if (rule.Right.StartsWith(pfx))
                        {
                            ...
                            result.Add(new CfgMessage(CfgErrorLevel.Message, -1,
                                string.Format("Removing rule {0} because it is part of a first-first conflict.",
                                    rule)));
                            Rules.Remove(rule);
                        }
                    ...
                    result.Add(new CfgMessage(CfgErrorLevel.Message, -1,
                        string.Format("Adding rule {0} to resolve first-first conflict.", newRule)));
        ...
        public IList<CfgMessage> EliminateLeftRecursion()
        {
            ...
            for (var i = 0; i < ic; ++i)
            {
                ...
                {
                    result.Add(new CfgMessage(CfgErrorLevel.Message, -1,
                        string.Format("Removing rule {0} because it is directly left recursive.", rule)));
                    Rules.Remove(rule);
                    ...
                    result.Add(new CfgMessage(CfgErrorLevel.Message, -1,
                        string.Format("Adding rule {1} to replace rule {0}", rule, newRule)));
                    if (!Rules.Contains(newRule))
                        Rules.Add(newRule);
                    var rr = new CfgRule(newId);
                    result.Add(new CfgMessage(CfgErrorLevel.Message, -1,
                        string.Format("Adding rule {1} to replace rule {0}", rule, rr)));
        ...
        public IList<CfgMessage> PrepareLL1(bool throwIfErrors = true)
        {
            ...
            if (IsDirectlyLeftRecursive)
                result.Add(new CfgMessage(CfgErrorLevel.Error, 9,
                    "Grammar is unresolvably and directly left recursive and cannot be parsed with an LL parser."));
            else if (IsLeftRecursive())
                result.Add(new CfgMessage(CfgErrorLevel.Error, 10,
                    "Grammar is unresolvably and indirectly left recursive and cannot be parsed with an LL parser."));
            var fc = FillFirstFollowsConflicts();
            foreach (var f in fc)
                result.Add(new CfgMessage(CfgErrorLevel.Error, 11,
                    string.Concat("Grammar has an unresolvable first-follows conflict on ", f.Key)));
            var c = FillFirstFirstConflicts();
            foreach (var f in c)
            foreach (var ff in f.Value)
                result.Add(new CfgMessage(CfgErrorLevel.Error, 12,
                    string.Concat("Grammar has an unresolvable first-first conflict between ", f.Key, " and ", ff)));
            if (throwIfErrors)
        ...
        public IDictionary<string, ICollection<string>> FillFollows(
            IDictionary<string, ICollection<string>> firsts = null,
            IDictionary<string, ICollection<string>> result = null)
        {
            if (null == result)
            ...
            var start = new CfgRule(GetTransformId(ss), ss, "#EOS");
            foreach (var rule in new[] { start }.Concat(Rules))
                if (rule.IsNil)
                {
                    ICollection<string> col;
                    if (!result.TryGetValue(rule.Left, out col))
        ...
        private ICollection<string> _FillLeftClosure(string nonTerminal, ICollection<string> result = null)
        {
        ...
        private ICollection<string> _FillLeftDescendants(string nonTerminal, ICollection<string> result = null)
        {
            if (null == result) result = new HashSet<string>();
            foreach (var rule in FillNonTerminalRules(nonTerminal))
                if (!rule.IsNil)
                        ...
                        foreach (var s in pre.Value)
                            if (null != s)
                            {
                                ...
                                if (null != (ir2 = arr[sid - nts.Count]).Right)
                                    throw new Exception(string.Format("Conflict between {0} and {1}",
                                        _ToStringIntRule(ir2), _ToStringIntRule(ir)));
        ...
        public TableDrivenLL1Parser ToLL1Parser(FA lexer, ParseContext parseContext = null)
        {
            ...
            for (var i = 0; i < substitutionsAndHiddenTerminals.Length; i++)
            {
                ...
                var d = AttributeSets.TryGetValue(symbols[i]);
                if (null != d)
                {
                    object o;
                    ...
                    {
                        var s = d.TryGetValue("substitute") as string;
                        if (null != s)
            ...
            for (var i = 0; i < blockEnds.Length; i++)
            {
                blockEnds[i] = null;
                var d = AttributeSets.TryGetValue(symbols[i]);
                if (null != d) blockEnds[i] = d.TryGetValue("blockEnd") as string;
            }
            ...
            for (var i = 0; i < terminalTypes.Length; i++)
            {
                ...
                if (null != d)
                {
                    var s = d.TryGetValue("type") as string;
                    if (null != s) terminalTypes[i] = ParserUtility.ResolveType(s);
                }
            }
        ...
        private string _ToStringIntRule((int Left, int[] Right) rule)
        {
            var sb = new StringBuilder();
        ...
        private static string _MakeSafeCsv(string field)
        {
            if (-1 < field.IndexOfAny(new[] { ',', '\"', '\n', '\r' }))
        ...
        {
            if (!CollectionUtility.Equals(Rules, rhs.Rules))
                return false;
